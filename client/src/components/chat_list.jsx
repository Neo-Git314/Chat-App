...
// --- Mock Data ---
const ONLINE_USERS = [
  { id: 1, name: 'Sarah', initials: 'SC', color: 'bg-[#d93fa5]' ,  isOnline: true,},
...
const StatusDot = ({ isOnline, className = "" }) => (
  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#130a1e] ${isOnline ? 'bg-[#5cd228]' : 'bg-[#6b627d]'} ${className}`}></div>
);

const Avatar = ({ initials, color, isOnline, size = "w-10 h-10" }) => (
  <div className="relative">
    <div className={`${size} ${color} rounded-full flex items-center justify-center text-white font-semibold text-sm`}>
      {initials}
    </div>
  ...
  return (
    <div className="w-[340px] h-screen bg-[#130a1e] flex flex-col font-sans -ml-2 z-10">
      ...
      <div className="p-5 pb-4 flex items-center gap-3 text-white">
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 text-[#a379f8]">
      ...
      <div className="px-5 pb-6">
        <div className="relative">
          <svg className="w-5 h-5 absolute left-3 top-2.5 text-[#89819e]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="px-5 py-2">
          <h2 className="text-[#89819e] text-xs font-bold uppercase tracking-wider mb-2">Messages</h2>
        </div>
      ...
      <div className="border-t border-[#261c33] p-4 px-5">
        <div className="flex items-center gap-3 cursor-pointer group">
          <Avatar initials="RR" color="bg-[#4a5568]" isOnline={true} size="w-10 h-10" />
          <div className="flex flex-col">
            <h3 className="text-white font-semibold text-sm group-hover:text-gray-200 transition-colors">Robin ray</h3>
